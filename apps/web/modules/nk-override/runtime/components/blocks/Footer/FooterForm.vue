  ...
  <div class="footer-settings-view sticky" data-testid="footer-settings-drawer">
    ...
    >
      ...
      <div class="py-2">
        ...
        <SfTextarea
          v-model="footerContent.column1.description"
          name="description"
          type="text"
    ...
    >
      ...
      <div class="py-2">
        ...
        <SfTextarea
          v-model="footerContent.footnoteDescription"
          name="description"
          type="text"
          class="w-full min-h-[232px]"
...
const footerContent = computed(() => footerBlock.value?.content as NkFooterContent);
  ...
  model: computed({
    ...
    set: (value: boolean) => {
      if (footerBlock.value?.content) {
        (footerBlock.value.content as NkFooterContent).column1[switchConfig.key] = value;
      }
    },
  }),
}));
...
const columnTwoSwitches = FOOTER_SWITCH_DEFINITIONS.filter((config) => config.columnGroup === 'services').map(
  (switchConfig) => ({
    ...
    model: computed({
      ...
      set: (value: boolean) => {
        if (footerBlock.value?.content) {
          (footerBlock.value.content as NkFooterContent).column2[switchConfig.key] = value;
        }
      },
    }),
  }),
